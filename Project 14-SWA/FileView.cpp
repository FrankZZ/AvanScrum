...
#define __FIRST_CREATE

CFileView::CFileView()
{
    CString             sText;
    std::string         name    = "testje";
    Sprint             *s;
    Defect             *d1,
                       *d2;
    SprintBacklogItem*  b;

#ifdef __FIRST_CREATE
    Project            *p;
    ...
    sText = CString("Bert ") + CString(User::isPresent("Bert")?"exists and " : "does not exist and ") +
            CString("Jos ")  + CString(User::isPresent("Jos") ?"exists " : "does not exist");
